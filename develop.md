...
- 修复：①新增 `_completed_days`：收盘结算时点（18:00）前把"今天"从目标交易日剔除（统计与抓取都不纳入）；②结算后即使今天仍拉不到也不记账、不进 suspect，留待次日以历史日身份重试（防 gm 数据滞后造成污染）。
- 清理：删除 no_data 中 2026-08-06 的 5205 条误记；复查过去日期 anomaly 无覆盖仅 5 条（302132 代码变更过渡日 4 条 + 300214 06-29 1 条，均非确认漏抓，30 天复核会再验证）。
- 保险检查（rebuild 内置）：audit_no_data 按日审计 anomaly/unknown 记账，超过阈值 max(50, valid×1%) 判定该天清单存疑，dry_run 报告、非 dry_run 移除异常记账使其重抓（绝不静默漏抓）；CLI 支持 --no-audit 跳过，WebUI 重建按钮自动执行并展示结果。
- 阈值澄清（用户质疑"2016 一天一百多个异常"）：审计只统计 anomaly/unknown（有行情却没拿到），不统计 suspended/boundary/code_change（状态接口确认的合理无行情）——2016 停牌高发期一天几百条停牌不会触发。实测 no_data 189,520 条中 anomaly/unknown 仅 10 条（每天最大 1），10 年无一天超阈值；50/1% 触发线保守且有效（08-06 误记 5199 条可被抓到）。审计循环优化为 groupby。
- 08-05 前数据核对（2026-08-06，WebUI 运行中，只读 --dry-run --compare）：coverage 重建 11 年 / 10,666,577 行，漂移 0，异常审计"无存疑天数"——08-05 前行情无问题。
- 测试：新增 _completed_days 与 audit_no_data 用例，pytest 16 项通过。
...
- WebUI 性能优化：数据详情统计由"每次交互同步扫描"改为单次 scan_missing_summary（_plan 单次加载静态表/交易日/覆盖索引，覆盖分组一次完成）+ st.cache_data(ttl=600) + @st.fragment(parallel=True) 后台并行计算（页面标题/侧边栏/进度/日志立即渲染，统计区显示 spinner 后填充）；任务规划阶段输出"扫描中 X/2572 天"进度（phase=scan），抓取阶段 phase=fetch；新增"刷新统计"按钮。首次统计约 11s（10.6M 覆盖行），10 分钟内刷新即时；实测当时数据缺失 0（回补已完成）。
