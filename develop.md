...
- 数据源覆盖边界（实测修正）：掘金免费行情包含其数据源保留的全部退市股（元数据 21 只验证全通过，含 2001/2008/2018/2025/2026 各年案例）；但掘金数据源本身缺少 2019–2024 退市潮的大部分退市股（乐视/康得新/暴风/康美/神雾/长油等，元数据无、行情 1000）。2016 有效集合约 2427 只 vs 当年实际约 2800+，缺口即这些退市股。如需完整无幸存者偏差历史数据，需引入补充数据源（如 akshare）。
