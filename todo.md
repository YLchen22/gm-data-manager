...
- [ ] coverage 重建工具：新增 data/rebuild_coverage —— 只读 bars 的 (date, symbol) 列全量重建 coverage 年度文件（原子覆盖写、去重），CLI 支持 --asset/--dry-run/--compare；回补完成后跑一次 --compare 验证漂移=0
- [ ] 空补记账（coverage 标记 + 分类准入 + 异常复核）：no_data 独立账本，coverage 保持"可从 bars 重建的纯投影"；只对确认停牌（is_suspended=1）/退市日/上市日/连续 3 次无状态记录（代码变更）写 no_data；真异常（有行情未返回）保持重试 + 短复核；状态接口失败时不标记
- [ ] 命令行动态输出统一：市场全量与截面增量的 CLI print/任务日志均改为三段式（待补 → 已入库 + 空补，未返回再拆停牌/异常/边界），去掉"失败"字样
- [ ] 数据服务 v0.9 完成后，完成情况总结移入 develop.md（含 --compare 漂移结果）
